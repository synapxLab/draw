// ── @synapxlab/draw — éditeur SVG autonome, zéro dépendance ─────────────────

export interface DrawEditorInterface {
...
// ── Classe principale ────────────────────────────────────────────────────────
export class DrawEditor implements DrawEditorInterface {
  ...
  private undo(): void {
    ...
    this.history.pop();
    // Les snapshots contiennent déjà les <defs> — pas besoin d'appeler initDefs()
  ...
  private bindEvents(): void {
    ...
    this.svg.addEventListener('touchstart', (e) => {
      ...
      this.onDown(this.touchToMouse(t, 'mousedown'));
    }, { passive: false });
    this.svg.addEventListener('touchmove', (e) => {
      e.preventDefault();
      const t = e.touches[0]; if (!t) return;
      this.onMove(this.touchToMouse(t, 'mousemove'));
    }, { passive: false });
    this.svg.addEventListener('touchend', (e) => {
      e.preventDefault();
      const t = e.changedTouches[0]; if (!t) return;
      this.onUp(this.touchToMouse(t, 'mouseup'));
    }, { passive: false });

    // Keyboard — stored reference so destroy() can remove it
  ...
  private touchToMouse(t: Touch, type: string): MouseEvent {
...
/** @deprecated Use DrawEditor */
export { DrawEditor as WysiwygDrawEditor };
